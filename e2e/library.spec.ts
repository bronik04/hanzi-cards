...
test('две колоды не мешают тренировкам друг друга', async ({ page }) => {
  ...
  // проверяется обратное — что колода помнит прогресс, — поэтому уходим на
  // импорт второй колоды напрямую, ссылкой «Загрузить новую таблицу»: она
  // ведёт на экран импорта, не трогая активную колоду.
  await page.getByRole('button', { name: 'Загрузить новую таблицу' }).click();
